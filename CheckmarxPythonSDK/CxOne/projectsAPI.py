...
class ApiData:
    def __init__(
        self, request_data: RequestData, response_handler: Callable[[dict], Any] = None
    ):
...
class ProjectsAPI(object):
    ...
    def create_a_project(self, project_input: ProjectInput) -> Project:
        ...
        response = self.api_client.post_request(
            relative_url=relative_url, json=project_input.to_dict()
        )
        return construct_project(response.json())

    def get_a_list_of_projects_api_data(
        self,
        offset: int = 0,
        limit: int = 200,
        ids: List[str] = None,
        ...
        return ApiData(
            request_data=request_data, response_handler=construct_projects_collection
        )
    ...
    def update_project_group(self, project_id: str, groups: List[str]) -> bool:
        ...
        response = self.api_client.put_request(
            relative_url=relative_url, json=project_data
        )
        return response.status_code == NO_CONTENT
...
def get_branches(
    offset: int = 0, limit: int = 200, project_id: str = None, branch_name: str = None
) -> List[str]:
    return ProjectsAPI().get_branches(
        offset=offset, limit=limit, project_id=project_id, branch_name=branch_name
    )
