    ...
    def from_dict(cls, item: dict) -> "ScaContainersCounters":
        ...
        return cls(
            ...
            total_vulnerabilities_counter=item.get(
                "totalVulnerabilitiesCounter"
            ),
            severity_vulnerabilities_counters=item.get(
                "severityVulnerabilitiesCounters"
            ),
            state_vulnerabilities_counters=item.get(
                "stateVulnerabilitiesCounters"
            ),
            status_vulnerabilities_counters=item.get(
                "statusVulnerabilitiesCounters"
            ),
            age_vulnerabilities_counters=item.get(
                "ageVulnerabilitiesCounters"
            ),
            package_vulnerabilities_counters=item.get(
