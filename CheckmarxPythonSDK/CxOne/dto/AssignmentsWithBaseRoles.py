...
class AssignmentsWithBaseRoles:
    ...
    entity_roles: List[str] = (
        None  # The roles assigned to the entity for this resource. Note: If no role is assigned,
    )
    # then the default "base" role is applied.
    resource_id: str = None  # The unique identifier of the resource
    resource_type: ResourceType = (
        None  # The type of resource [ application, project, tenant ]
    )
    resource_name: str = None  # Human-readable name of the resource
    effective_permissions_count: int = (
        None  # The number of effective permissions for the entity on the resource
    )
